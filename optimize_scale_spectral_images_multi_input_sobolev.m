...
cfa = cfa_calibration_weights(color,:)*resp_scale;

tol = 1e-12;
lb = zeros(size(cfa));
ub = max(cfa)*ones(size(cfa));
maxiter = 10000;

xsol(color,:) = lbfgsb(cfa, lb(:), ub(:), ...
    'obj_scale_factor_multi_input_sobolev', 'calib_gradfun', ...
    ax, [], ...
    'maxiter', maxiter, ...
    'm', 3, ...
    'factr', tol, ...
...
%%
xsol = cfa_calibration_weights;
save('/home/chenjiee/Projects/code_public/slm_camera/multispectral_deconvolution_matlab/refine_cfa.mat','cfa_calibration_weights');